...
class Minio:  # pylint: disable=too-many-public-methods
    ...
    **NOTE on concurrent usage:** `Minio` object is thread safe when using
    ...
    def list_files_in_directory(self, bucket_name, directory):
        objects = self.list_objects(bucket_name, prefix=directory, recursive=True)
        file_names = []
        for obj in objects:
            name = os.path.relpath(obj.object_name, directory)
            name = name.replace(".pkl", "")
            file_names.append(name)
        return file_names
 
    def _save_state_dict(self, state_dict, path, **kwargs):
    ...
    def _save_info(self, state_dict, path, **kwargs):
    ...
    def log_artifacts(self, bucket, local_dir, artifact_path=None):
        for root, _, filenames in os.walk(local_dir):
            ...
            #     self.put_object(bucket_name=bucket, object_name=upload_path + "/")

            for f in filenames:
                self.fput_object(
                    bucket_name=bucket,
                    file_path=os.path.join(root, f),
                    object_name=os.path.join(upload_path, f),
                    part_size= 500*1024*1024
                )
        return 

    def log_model_artifacts(self, bucket, local_dir, artifact_path=None):
        model_path = os.path.join(artifact_path, "model")
        for root, _, filenames in os.walk(local_dir):
            upload_path = model_path
            if root != local_dir:
                rel_path = os.path.relpath(root, local_dir)
                upload_path = os.path.join(model_path, rel_path)
    ...
    def cput_object(self, bucket_name, object_name, file_path, s3_client=None):
        """
        This function is for uploading model by layers.
        Same layer will not be uploaded again.
        """
        loaded_model_state = torch.load(file_path, map_location='cpu')
        # etag = self.hash_cal(loaded_model_state)

        # for k, v in loaded_model_state.items():
        #     hash_name = self.hash_cal(v)
        #     key = object_name + "/" + hash_name + ".pkl"
        #loaded_model_state = torch.load(file_path)
        file_list = self.list_files_in_directory(bucket_name=bucket_name, directory=object_name+"/model")
        # for file_name in file_list:
        #     print(file_name)
        try:
            with TempDir() as tmp:
                local_path = tmp.path()
                self._save_state_dict(state_dict=loaded_model_state, path=local_path, file_list=file_list)
                self.log_model_artifacts(bucket_name, local_path, object_name)
            with TempDir() as tmp:
                local_path = tmp.path()
                self._save_info(state_dict=loaded_model_state, path=local_path)
                self.log_artifacts(bucket_name, local_path, object_name)
        except Exception as e:
            print("文件上传异常:", e)

            # try:

            #     # s3_client.upload_file(v, bucket_name, str(key))

            #     # 创建临时文件
            #     with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            #         # 将 Tensor 对象保存到临时文件中
            #         torch.save(v, tmp_file)
            #         tmp_file.flush()  # 刷新缓冲区以确保所有数据写入文件
            ...
            #         while True:
            #             data = tmp_file.read(5000 * 1024 * 1024)  # 读取 5MB 数据作为一个分片（可根据需求调整大小）
            #             if not data:
            #                 break
        ...
        return ObjectWriteResult(
            ...
            #     while True:
            #         data = file.read(5000 * 1024 * 1024)  # 读取 5MB 数据作为一个分片（可根据需求调整大小）
            #         if not data:
            #             break
            #
            #         # 上传分片
            ...
            #
            # # 完成分片上传
            # # parts = [{'PartNumber': i, 'ETag': response['ETag']} for i in range(1, part_number)]
            # result = s3_client.complete_multipart_upload(Bucket=bucket_name,
            #                                     Key=key,
            #                                     UploadId=upload_id,
            #                                     MultipartUpload={'Parts': parts})
            # return ObjectWriteResult(
    ...
    def fput_object(self, bucket_name, object_name, file_path,
                    ...
                    tags=None, retention=None, legal_hold=False,_s3_client=None):
        """
        ...
        :param _s3_client: Server-side encryption.
        ...
        with open(file_path, "rb") as file_data:
            return self.put_object(
                ...
                tags=tags, retention=retention, legal_hold=legal_hold,_s3_client=_s3_client
            )


    def download_model_from_info(self,bucket,object_name,output_path):
        parts = object_name.split('/')
        last_part = parts[-1]
        info_path = os.path.join(output_path,last_part)
        self.fget_object(bucket_name=bucket,object_name=object_name,file_path=info_path)
        hash_value = model_compose.get_hash(info_path)
        model_path_dir = '/'.join(parts[:-1])+'/model'
        for i in hash_value:
            artifact_path = model_path_dir+'/'+i+'.pkl'
            # print(artifact_path)
            self.fget_object(bucket_name=bucket,object_name=artifact_path,file_path=os.path.join(output_path,"model",i+'.pkl'))
    ...
    def cget_object(self, bucket_name, object_name, file_path=None,
                    request_headers=None, ssec=None, version_id=None,
                    extra_query_params=None, tmp_file_dir=None):
        """

        """
        check_bucket_name(bucket_name)
        check_non_empty_string(object_name)
        # state_dict_uri = "mlflow-artifacts:/1/fe32d834d3394f338954cda787594f84/artifacts"
        

        with TempDir() as tmp:
            output_path = tmp.path()
            local_path = self.download_model_from_info(bucket=bucket_name, object_name=object_name, output_path=output_path)
    ...
    def fget_object(self, bucket_name, object_name, file_path,
        ...
        # Write to a temporary file "file_path.part.minio_test" before saving.
        tmp_file_path = (
            tmp_file_path or file_path + "." + stat.etag + ".part.minio_test"
        )
        try:
    ...
    def put_object(self, bucket_name, object_name, data, length,
        ...
        :param retention: :class:`Retention` configuration object.
        :param legal_hold: Flag to set legal hold for the object.
        :param _s3_client: Flag to set legal hold for the object.
        :return: :class:`ObjectWriteResult` object.
